...
#define CTRL_SAMPLE_PERIOD_TICKS		(unsigned long)( 10.0 * 1000000.0 / (float)PWM_PERIOD_US )        //  58593
...
#define CTRL_ABOUT_TO_SAMPLE_TICKS		(unsigned long)( 8.0 * 1000000.0 / (float)PWM_PERIOD_US )     // 54687
...
typedef struct Ctrl_
{
	...
	int pwmDisabledByOnOff; //RDD set on IO_getGroundFault()  //RDD togle by buttom?
...
void CTRL_calcOutVoltSetpoints()//RDD Calls every 2s
{
	int flsetDec;
...
int CTRL_isGroundFault()  // RDD not used anywhere
{
	return ctrl.pwmGroundFaultShutdown;  //RDD condition with hystiresis
}
...
//			ctrl.pwmShutdown = 1;
	...
	*/
	...
	if ( ctrl.tickCount == CTRL_SAMPLE_PERIOD_TICKS ) // RDD  58593
	{
		ctrl.tickCount = 0;
	}
